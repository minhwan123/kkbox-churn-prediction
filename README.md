...
| 개별 고객 조회 | 확률 게이지 + 고객 프로필 비교 |
|---|---|
| ![overview](docs/screenshots/01_overview.png) | ![gauge](docs/screenshots/02_prediction_gauge.png) |

| 고객 메시지 미리보기 + SHAP 설명 | 배치 예측 전체 현황 |
|---|---|
| ![shap](docs/screenshots/04_customer_message_shap.png) | ![batch](docs/screenshots/05_batch_overview.png) |
...
- **진짜 데이터**: 합성/더미 데이터가 아닌 [KKBox WSDM Cup 2018 Kaggle 대회](https://www.kaggle.com/c/kkbox-churn-prediction-challenge) 데이터 (실사용자 97만 명, 결제 이력 1,600만 건, 청취 로그 수억 건)
- **엔드투엔드**: EDA → 전처리 → 모델링 → API 서버 → 웹 UI까지 전 과정을 직접 구현
